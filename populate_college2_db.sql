...
create table _temp_fr_en_etablissements_ep(
  ...
  type_etablissement      varchar(39), --_departement libelle_nature
...
ALTER TABLE _quartier_prioritaire
  DROP CONSTRAINT quartier_prioritaire_pk CASCADE;
...
insert into _quartier_prioritaire (code_quartier_prioritaire, nom_quartier_prioritaire)
  ...
  from _temp_fr_en_etablissements_ep
  where (qp_a_proximite is not null) AND (nom_du_qp is not null) AND 
         (qp_a_proximite NOT IN (Select qp_a_proximite from _quartier_prioritaire));
...
set nom_commune = upper(nom_commune);
...
insert into _etablissement (uai, nom_etablissement, secteur, code_postal, lattitude, longitude, code_insee_de_la_commune, nom_de_la_commune, code_academie, code_nature)
  ...
  from _temp_fr_en_etablissements_ep
  where (uai is not null) 
    AND (nom_etablissement is not null) 
    AND (code_postal is not null) 
    AND (latitude is not null) 
    AND (longitude is not null)
    AND (code_commune, nom_commune) in (SELECT code_insee_de_la_commune, nom_de_la_commune FROM _commune);

----------------
-- Caracteristiques tout établissement
...
insert into _caracteristiques_tout_etablissement (uai, annee_scolaire, effectifs)
  select distinct uai, rentree_scolaire, effectifs
  from _temp_fr_en_ips_colleges_ap2022
  WHERE uai in (SELECT uai from _etablissement) AND rentree_scolaire in (SELECT annee_scolaire from _annee);
...
ADD IF NOT EXISTS nom_quartier_prioritaire varchar(85);

ALTER TABLE _est_a_proximite_de
ADD constraint est_a_proximite_de_fk_qp
...
insert into _est_a_proximite_de (code_quartier_prioritaire, uai, nom_quartier_prioritaire)
  ...
  from _temp_fr_en_etablissements_ep
  where (qp_a_proximite, nom_du_qp) in (SELECT code_quartier_prioritaire, nom_quartier_prioritaire FROM _quartier_prioritaire) AND uai in (SELECT uai from _etablissement);

----------
--- Caractéritisques college
...
INSERT INTO _caracteristiques_college (uai, annee_scolaire, nbre_eleves_hors_segpa_hors_ulis, nbre_eleves_segpa, nbre_eleves_ulis, ips, ecart_type_de_l_ips, ep)
  SELECT DISTINCT ep.uai, lv.rentree_scolaire, (nombre_eleves_total - nombre_eleves_segpa - nombre_eleves_ulis) nbre_eleves_hors_segpa_hors_ulis, nombre_eleves_segpa, nombre_eleves_ulis, ips, ecart_type_de_l_ips, 
    CASE ep.ep_2022_2023
      WHEN 'REP' THEN true
      WHEN 'REP+' THEN true
      WHEN 'HORS EP' THEN false
      ELSE false
    END
    
  FROM _temp_fr_en_ips_colleges_ap2022 ips INNER JOIN
  _temp_fr_en_etablissements_ep ep ON ep.uai = ips.uai INNER JOIN 
  _temp_fr_en_college_effectif_niveau_sexe_lv lv ON lv.numero_college = ep.uai
  WHERE ep.uai in (SELECT uai from _etablissement);   

----------
--- Caractéritisques selon classe
delete from _classe;
...
INSERT INTO _caracteristiques_selon_classe(id_classe, uai, annee_scolaire, nbre_eleves_hors_segpa_hors_ulis_selon_niveau, nbre_eleves_segpa_selon_niveau, nbre_eleves_ulis_selon_niveau, effectifs_filles, effectifs_garcons)
  SELECT DISTINCT id_classe, numero_college, rentree_scolaire, 
  CASE id_classe
      WHEN '6eme' THEN lv._6eme_total - lv._6eme_segpa - lv._6eme_ulis
      WHEN '5eme' THEN lv._5eme_total - lv._5eme_segpa - lv._5eme_ulis
      WHEN '4eme' THEN lv._4eme_total - lv._4eme_segpa - lv._4eme_ulis
      WHEN '3eme' THEN lv._3eme_total - lv._3eme_segpa - lv._3eme_ulis
      ELSE 0
  END,
  CASE id_classe
      WHEN '6eme' THEN lv._6eme_segpa
      WHEN '5eme' THEN lv._5eme_segpa
      WHEN '4eme' THEN lv._4eme_segpa
      WHEN '3eme' THEN lv._3eme_segpa
      ELSE 0
  END,
  CASE id_classe
      WHEN '6eme' THEN lv._6eme_ulis
      WHEN '5eme' THEN lv._5eme_ulis
      WHEN '4eme' THEN lv._4eme_ulis
      WHEN '3eme' THEN lv._3eme_ulis
      ELSE 0
  END,
  CASE id_classe
      WHEN '6eme' THEN lv._6eme_filles
      WHEN '5eme' THEN lv._5eme_filles
      WHEN '4eme' THEN lv._4eme_filles
      WHEN '3eme' THEN lv._3eme_filles
      ELSE 0
  END,
  CASE id_classe
      WHEN '6eme' THEN lv._6emes_garcons
      WHEN '5eme' THEN lv._5emes_garcons
      WHEN '4eme' THEN lv._4emes_garcons
      WHEN '3eme' THEN lv._3emes_garcons
      ELSE 0
  END 

  FROM _temp_fr_en_college_effectif_niveau_sexe_lv lv INNER JOIN 
          _etablissement et ON lv.numero_college = et.uai INNER JOIN
          _annee an ON an.annee_scolaire = lv.rentree_scolaire CROSS JOIN 
          _classe;
commit;
